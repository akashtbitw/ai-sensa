// server/routes/fallDetection.js
const express = require("express");
const router = express.Router();
const FallDetection = require("../models/FallDetection");

// Get all fall detection data for a user
router.get("/:userId", async (req, res) => {
  try {
    ...
    if (!startDate && !endDate) {
      ...
      fallData = await FallDetection.find(query)
      ...
      fallData = await FallDetection.find(query).sort({ timestamp: 1 });
    }

    res.json(fallData);
  } catch (err) {
    console.error("Error fetching fall detection data:", err);
